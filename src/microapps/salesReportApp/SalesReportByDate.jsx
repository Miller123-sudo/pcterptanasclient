...
export default function SalesReportByDate() {
    ...
    const salesReportByWeek = async () => {
        if (getValues("startDate") > getValues("endDate")) {
            ...
            const response = await ApiService.get(`salesOrder/findSalesByWeek?startDate=${getValues("startDate")}&endDate=${getValues("endDate")}`);
            if (response.data.isSuccess) {
                console.log(response.data.documents)
                setstate(response.data.documents)
    ...
    useEffect(async () => {
        ...
        console.log(getValues())
        const response = await ApiService.get(`salesOrder/findSalesByDay`);
        if (response.data.isSuccess) {
            console.log(response.data.documents)
            setstate(response.data.documents)
                ...
                <Container className="pct-app-content-header p-0 m-0 pb-2" fluid>
                    <Row>
                        <Col className='p-0 ps-2'>
                            <Breadcrumb style={{ fontSize: '24px', marginBottom: '0 !important' }}>
                                <Breadcrumb.Item active> <div className='breadcrum-label-active'>SALES REPORT FOR DAY, WEEK, MONTH OR YEAR</div></Breadcrumb.Item>
                    ...
                    <Row style={{ marginTop: '-10px' }}>
                        ...
                        <Col className='p-0 ps-1'>
                            ...
                            <Button onClick={salesReportByWeek} variant="primary" size="sm"><span>GET ORDERS</span></Button>
                            {/* <Button onClick={salesReportByWeek} variant="primary" size="sm"><span>BY WEEK</span></Button>
                            <Button onClick={salesReportByMonth} variant="primary" size="sm"><span>BY MONTH</span></Button>
                            <Button onClick={salesReportByYear} variant="primary" size="sm"><span>BY YEAR</span></Button> */}
                        </Col>
                        <Col md="4" sm="4">
                            <Row>
